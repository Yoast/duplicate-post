...
# Output format matches the Duplicate Post plugin's changelog.md structure.
#
...
for PR_NUM in $PR_NUMBERS; do
	...
	while IFS= read -r line; do
		...
		# Remove [duplicate-post] scope prefix if present.
...
for META_FILE in readme.txt duplicate-post.php; do
	if [ -f "$META_FILE" ]; then
...
HEADER="Yoast Duplicate Post
=========
Requires at least: ${REQUIRES_AT_LEAST:-6.8}
...
if [ -f "$CHANGELOG_FILE" ]; then
	...
	# Args: $1 = section version string (e.g., "4.6.1").
	should_keep_section() {
